...
<table width="798" class="border" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  if(isset($_GET['id'])){
    ...
    $id = $_GET['id'];
    $cek = mysql_query("SELECT id_pengaduan FROM pengaduan WHERE id_pengaduan = '$id'") or die(mysql_error());
    if(mysql_num_rows($cek) == 0){
      echo "<script>window.history.back()</script>";
    }   
    else{
      $del = mysql_query("DELETE FROM pengaduan WHERE id_pengaduan = '$id'");
      if($del){
